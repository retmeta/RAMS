...
with st.expander("Klik om de wiskundige formules te bekijken die gebruikt worden in de berekeningen"):
    st.markdown("""
...
if 'components' not in st.session_state:
    st.session_state.components = {
        "Repeater": {"MTBF": 100000, "MTTR_no_SLA": 72, "MTTR_with_SLA": 12},
        "Voeding/UPS": {"MTBF": 50000, "MTTR_no_SLA": 48, "MTTR_with_SLA": 8},
        "Glasvezel": {"MTBF": 100000, "MTTR_no_SLA": 24, "MTTR_with_SLA": 24},
        "Omgevingsfactoren": {"MTBF": 50000, "MTTR_no_SLA": 48, "MTTR_with_SLA": 10}
    }
...
sla_cost_total = st.sidebar.number_input(
    "Totale SLA kosten (5 jaar) (€)",
    min_value=100,
    max_value=20000,
    value=8665,
    step=50
...
inflation_rate = st.sidebar.number_input(
    "Jaarlijkse inflatie percentage (%)",
    min_value=0.0,
    max_value=10.0,
    value=4.5,
    step=0.1,
...
seasonal_operation = st.sidebar.checkbox(
    ...
    value=True,
    help="Activeer deze optie als de locatie alleen tijdens het strandseizoen operationeel is"
)
...
if seasonal_operation:
    season_start_month = st.sidebar.selectbox(
        ...
        format_func=lambda x: ["", "", "", "", "April", "Mei", "Juni", "Juli"][x]
    )
    ...
    season_end_month = st.sidebar.selectbox(
        ...
        format_func=lambda x: ["", "", "", "", "", "", "", "", "Augustus", "September", "Oktober", "November"][x]
    )
    ...
    st.sidebar.info(f"Operationeel: {season_months} maanden per jaar ({seasonal_factor:.1%} van het jaar)")
else:
    seasonal_factor = 1.0
    season_months = 12
...
inflation_factor = (1 + inflation_rate / 100) ** 3  # Compound inflation for year 3
...
# Simulation function
@st.cache_data
def simulate(components_dict, mttr_key, sla_active, n_sims, sla_yearly_cost, callout, engineer_hourly, seasonal_factor=1.0):
    ...
    for _ in range(n_sims):
        ...
        for component_name, vals in components_dict.items():
            ...
            if failures > 0:
                ...
                total_cost += failures * callout + sum(repair_times) * engineer_hourly
...
if st.sidebar.button("Start Simulatie", type="primary"):
    with st.spinner("Monte Carlo simulatie wordt uitgevoerd..."):
        ...
        downtime_no_SLA, costs_no_SLA = simulate(
            ...
            sla_cost_per_year, callout_fee_no_sla_indexed, engineer_cost_per_hour_no_sla_indexed, seasonal_factor
        )
        
        downtime_with_SLA, costs_with_SLA = simulate(
            components, "MTTR_with_SLA", True, n_simulations, 
            sla_cost_per_year, callout_fee_sla_indexed, engineer_cost_per_hour_sla_indexed, seasonal_factor
        )
...
if hasattr(st.session_state, 'simulation_complete') and st.session_state.simulation_complete:
    ...
    with col2:
        ...
        cost_data = [
            ...
            ["Oproepkosten Met SLA (geïndexeerd)", f"€{callout_fee_sla_indexed:.2f}"],
            ["Oproepkosten Zonder SLA (basis)", f"€{callout_fee_no_sla}"],
            ["Oproepkosten Zonder SLA (geïndexeerd)", f"€{callout_fee_no_sla_indexed:.2f}"],
            ["Monteur Met SLA (basis)", f"€{engineer_cost_per_hour_sla}/uur"],
            ["Monteur Met SLA (geïndexeerd)", f"€{engineer_cost_per_hour_sla_indexed:.2f}/uur"],
            ["Monteur Zonder SLA (basis)", f"€{engineer_cost_per_hour_no_sla}/uur"],
            ["Monteur Zonder SLA (geïndexeerd)", f"€{engineer_cost_per_hour_no_sla_indexed:.2f}/uur"],
            ["Seizoensgebonden operatie", "Ja" if seasonal_operation else "Nee"],
    ...
    hist_no_sla, bin_edges_no_sla = np.histogram(st.session_state.downtime_no_SLA, bins=50)
    mode_bin_no_sla = np.argmax(hist_no_sla)
    mode_downtime_no_sla = (bin_edges_no_sla[mode_bin_no_sla] + bin_edges_no_sla[mode_bin_no_sla + 1]) / 2
    
    hist_with_sla, bin_edges_with_sla = np.histogram(st.session_state.downtime_with_SLA, bins=50)
    mode_bin_with_sla = np.argmax(hist_with_sla)
    mode_downtime_with_sla = (bin_edges_with_sla[mode_bin_with_sla] + bin_edges_with_sla[mode_bin_with_sla + 1]) / 2
    ...
    with col1:
        ...
        prob_data_no_sla = [
            ...
            ["Meest waarschijnlijke waarde", f"{mode_downtime_no_sla:.1f} uren"]
        ]
        
        prob_df_no_sla = pd.DataFrame(prob_data_no_sla, columns=['Kans', 'Downtime'])
    ...
    with col2:
        ...
        prob_data_with_sla = [
            ...
            ["Meest waarschijnlijke waarde", f"{mode_downtime_with_sla:.1f} uren"]
        ]
        
        prob_df_with_sla = pd.DataFrame(prob_data_with_sla, columns=['Kans', 'Downtime'])
        st.dataframe(prob_df_with_sla, use_container_width=True, hide_index=True)
    ...
    fig_downtime.add_trace(go.Histogram(
        x=st.session_state.downtime_no_SLA,
        name="Zonder SLA",
        opacity=0.7,
        nbinsx=50,
        marker_color='red'
    ...
    fig_downtime.add_trace(go.Histogram(
        x=st.session_state.downtime_with_SLA,
        name="Met SLA",
        opacity=0.7,
        nbinsx=50,
        marker_color='green'
    ...
    fig_downtime.update_layout(
        title="Jaarlijkse Downtime Verdeling",
    ...
    cost_data = pd.DataFrame({
        'Scenario': ['Zonder SLA'] * len(st.session_state.costs_no_SLA) + ['Met SLA'] * len(st.session_state.costs_with_SLA),
        'Cost': np.concatenate([st.session_state.costs_no_SLA, st.session_state.costs_with_SLA])
    })
    
    fig_cost = px.box(cost_data, x='Scenario', y='Cost', 
    ...
    for mttr in mttr_range:
        ...
        _, cost_sim = simulate(
            components_temp, "MTTR_no_SLA", False, 1000, 
            sla_cost_per_year, callout_fee_no_sla_indexed, engineer_cost_per_hour_no_sla_indexed, seasonal_factor
        )
    ...
    fig_sensitivity = px.line(sensitivity_df, x='MTTR', y='Average Cost',
                             title="Gevoeligheidsanalyse: Repeater MTTR vs. Gemiddelde Kosten (Zonder SLA)",
                             markers=True)
    ...
    cost_diff = np.mean(st.session_state.costs_no_SLA) - np.mean(st.session_state.costs_with_SLA)
    ...
    with col2:
        ...
        summary_data = {
            'Statistiek': ['Gemiddelde', 'Mediaan', 'Std. Afwijking', '95e Percentiel'],
            'Zonder SLA (Downtime)': [
    ...
    with col1:
        ...
        results_df = pd.DataFrame({
            'Simulatie': range(1, len(st.session_state.downtime_no_SLA) + 1),
            'Downtime_Zonder_SLA': st.session_state.downtime_no_SLA,
            'Kosten_Zonder_SLA': st.session_state.costs_no_SLA,
            'Downtime_Met_SLA': st.session_state.downtime_with_SLA,
            'Kosten_Met_SLA': st.session_state.costs_with_SLA
        })
        
        csv = results_df.to_csv(index=False)
        st.download_button(
            label="Download Simulatie Resultaten (CSV)",
            data=csv,
...
"""
        ...
        st.download_button(
            label="Download Samenvatting Rapport (TXT)",
            data=summary_report,
    ...
    with col3:
        ...
        st.download_button(
            label="Download Gevoeligheidsanalyse (CSV)",
    ...
    st.info("Configureer parameters in de sidebar en klik op 'Start Simulatie' om de analyse te beginnen.")
    ...
    with col1:
        ...
        for comp_name, comp_data in components.items():
            input_summary_components.append({
                'Component': comp_name,
                'MTBF': f"{comp_data['MTBF']:,} uren",
                'MTTR (zonder SLA)': f"{comp_data['MTTR_no_SLA']} uren",
                'MTTR (met SLA)': f"{comp_data['MTTR_with_SLA']} uren"
            })
